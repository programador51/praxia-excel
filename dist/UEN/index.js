...
var file = (0, _convertExcelToJson["default"])({
  sourceFile: "".concat(__dirname, "/../../src/UEN/UEN.xlsx"),
  columnToKey: {
    A: "UENID",
    B: "description",
    C: "family",
...
var uenParsed = file["UEN"].reduce(function (list, item, i) {
  if (i === 0) return list;
  if (item.UENID === undefined || item.UENID === null || item.UENID === "" || typeof item.UENID !== "number") return list;
  return [].concat(_toConsumableArray(list), [_objectSpread(_objectSpread({}, item), {}, {
...
jsonexport(uenParsed, function (err, csv) {
  ...
  var writer = fs.createWriteStream("UEN.csv");
  writer.write(csv);
